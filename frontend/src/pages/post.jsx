...
const Post = () => {
    ...
    // Construct breadcrumb items based on URL parameters
    const breadcrumbItems = [
        { label: 'Pagrindinis', path: '/pagrindinis' }
    ];

    // Add forum context if available
    if (location.state?.forumName) {
        breadcrumbItems.push(
            { label: 'Forumai', path: '/forumai' },
            { label: 'Universitetai', path: '/forumai/universitetai' },
            { label: truncateText(location.state.forumName), path: `/forumai/universitetai/${params.universityId}` }
        );
    }

    // Add program context if available
    if (params.programId && location.state?.programName) {
        breadcrumbItems.push(
            { label: 'Programos', path: `/forumai/universitetai/${params.universityId}/programos` },
            { label: truncateText(location.state.programName), path: `/forumai/universitetai/${params.universityId}/programos/${params.programId}` }
        );
    }

    // Add category context if available
    if (params.categoryId && location.state?.categoryName) {
        breadcrumbItems.push(
            { label: 'Kategorijos', path: '/forumai/kategorijos' },
            { label: truncateText(location.state.categoryName), path: `/forumai/kategorijos/${params.categoryId}` }
        );
    }

    // Add current post
    if (post) {
        breadcrumbItems.push({ label: truncateText(post.title), path: location.pathname });
    }
                    ...
                    <div className="flex flex-col gap-4">
                        <div className="flex flex-row items-center justify-between gap-4">
                            <div className="flex flex-row gap-2 items-center">
                                <img src={profilePicture} className="size-8 md:size-10" alt="Profile" />
